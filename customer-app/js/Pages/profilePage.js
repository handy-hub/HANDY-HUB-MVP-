...
const DEFAULT_AVATAR =
  "https://ui-avatars.com/api/?background=730201&color=fff&size=128&name=User";
...
function buildDefaultAvatarUrl(name) {
  ...
  return `https://ui-avatars.com/api/?background=730201&color=fff&size=128&name=${encoded}`;
}
...
async function handlePhotoUpload(file) {
  ...
  const previousAvatarSrc = avatarImg ? avatarImg.src : DEFAULT_AVATAR;
  ...
  try {
    ...
    await storageService.uploadFile(storagePath, file, { contentType: file.type });
    const downloadUrl = await storageService.getDownloadUrl(storagePath);

    await databaseService.setDocument(CUSTOMERS_COLLECTION, currentUserId,
      { profileImage: downloadUrl }, { merge: true });

    showToast("Profile photo updated!", "success");
  } catch (err) {
    console.error("Photo upload failed:", err);
    showToast("Failed to upload photo. Please try again.", "error");
    if (avatarImg) avatarImg.src = previousAvatarSrc;
  } finally {
...
authService.subscribeToAuthState((user) => {
  ...
  unsubscribeSnapshot = databaseService.subscribeToDocument(
